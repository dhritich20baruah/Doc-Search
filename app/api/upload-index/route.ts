...
export async function POST(request: Request) {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        ...
        return NextResponse.json({ message: "Server-side Supabase keys not configured (Check SUPABASE_SERVICE_ROLE_KEY)." }, { status: 500 });
    }

     const supabaseService = createClient(supabaseUrl as string, serviceRoleKey as string);

  try {
    const { fileName, base64Content, content, title } = await request.json();

    if (!fileName || !base64Content || !content) {
